...
public class ProjectManager {
	...
	public Map<Integer, Project> getProjects() { return _projects; }
	...
	public void setSelectedProject(int id) {
		if(!_projects.containsKey(id)) {
			System.err.println("Error: Trying to select a project with ID " + id + " -- not found!");
			return;
		}
		_selected_project = id;
	}
	
	public boolean hasSelected() { return _selected_project > 0; }
	...
	private void populateProjects() {
		try
		{
			...
			ResultSet rs = stmt.executeQuery( "SELECT id, name, description, date_created FROM projects, project_managers "
											+ "WHERE project_managers.manager_id == " + _owner.getId() 
											+ " AND projects.id == project_managers.project_id;");
			
			while(rs.next())
			{
				int id = rs.getInt("id");
				Project p = new Project(id, rs.getString("name"), rs.getString("description"), rs.getDate("date_created"));
				_projects.put(id, p) ;
			}
	...
	public void updateProjectName(String new_name) {
	    ...
	    {
			stmt = DB.getInstance().createStatement();
			String sql = "UPDATE projects SET name = '" + new_name + "' WHERE id = " + _selected_project + ";";
	...
	public void updateProjectDescription(String new_descr) {
	    ...
	    {
			stmt = DB.getInstance().createStatement();
			String sql = "UPDATE projects SET description = '" + new_descr + "' WHERE id = " + _selected_project + ";";
		    stmt.executeUpdate(sql);
	...
	public void deleteSelectedProject() {
	    ...
	    try 
	    {
		    ...
		    stmt.executeUpdate(sql);
		    sql = "DELETE FROM project_managers WHERE manager_id = " + _owner.getId() + " AND project_id = " + _selected_project + ";";
		    stmt.executeUpdate(sql);
	...
	public void addNewProject(String name, String description) 
	{
	    ...
	    try 
	    {
		    ...
		    sql = "INSERT INTO project_managers VALUES (" + proj_id + ", " + _owner.getId() + ")";
		    stmt.executeUpdate(sql);
		    
		    _selected_project = proj_id;
		    _projects.put(proj_id,new Project(proj_id, name, description, date));
